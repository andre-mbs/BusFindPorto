    ...
    stage('Runtime Deployment') { 
      steps {
          sshagent(credentials: ['esp13-sshagent']){
              ...
              sh "ssh -o 'StrictHostKeyChecking=no' -l esp13 192.168.160.103 docker run --log-driver json-file --log-opt max-file=5 --log-opt max-size=10m -p 11000:11080 --name esp13-service-layer 192.168.160.99:5000/esp13-service-layer"
